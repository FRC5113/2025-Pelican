...
class Twitch:
    ...
    def receive_and_parse_data(self):
        ...
        if buffer:
            ...
            # Parse irc messages
            res = []
            matches = list(self.re_prog.finditer(buffer)) # pyright: ignore[reportOptionalMemberAccess]
        ...
        for irc_message in self.receive_and_parse_data():
            ...
            if cmd == 'PRIVMSG':
                privmsgs.append({
                    ...
                    'message': re.sub(r'[\u200B-\u200F\u034F\uFEFF]', '', irc_message['trailing']).strip(),
                })
            elif cmd == 'PING':
                self.sock.send(b'PONG :tmi.twitch.tv\r\n') # pyright: ignore[reportOptionalMemberAccess]
            elif cmd == '001':
                print('Successfully logged in. Joining channel %s.' % self.channel)
                self.sock.send(('JOIN #%s\r\n' % self.channel).encode()) # pyright: ignore[reportOptionalMemberAccess]
                self.login_ok = True
            elif cmd == 'JOIN':
